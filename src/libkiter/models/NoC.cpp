		...
		NoC::NoC (int XSIZE, int YSIZE) : _XSIZE(XSIZE), _YSIZE(YSIZE) {
			...
			for (int X = 0 ; X < XSIZE ; X ++) {
				...
				for (int Y = 0 ; Y < YSIZE ; Y ++) {
					...
					NetworkEdge ep = NetworkEdge(core_id*NodeCount + source , core_id, source);
					...
					NetworkEdge e = NetworkEdge(source*NodeCount + core_id, source, core_id);
					...
					if (X < (XSIZE - 1)) {
						...
						NetworkEdge e = NetworkEdge(source*NodeCount + dest_right, source, dest_right);
						_mid2edges.insert({e.id,e});
						_medges.insert({std::pair<node_id_t,node_id_t>( source, dest_right),e});
						_vedges.push_back(e);

						NetworkEdge ep = NetworkEdge(dest_right*NodeCount + source, dest_right, source);
						_mid2edges.insert({ep.id,ep});
					...
					if (Y < (YSIZE - 1)) {
						...
						NetworkEdge e = NetworkEdge(source*NodeCount + dest_bottom, source, dest_bottom);
						_mid2edges.insert({e.id,e});
						_medges.insert({std::pair<node_id_t,node_id_t>( source, dest_bottom),e});
						_vedges.push_back(e);


						NetworkEdge ep = NetworkEdge(dest_bottom*NodeCount + source, dest_bottom, source);
						_mid2edges.insert({ep.id,ep});
